...
export async function findLeakingCollections(
  page,
  collectionsToCountsMap,
  numIterations,
  debug
  ...
  const leakingCollections = await page.evaluate(
    (
      objects,
      collectionsToCountsMap,
      trackedStacktraces,
  ...
  await Promise.all([
    prototype.dispose(),
    objects.dispose(),
    collectionsToCountsMap.dispose(),
  ]);
...
export async function augmentLeakingCollectionsWithStacktraces(
  page,
  collections,
  trackedStacktraces
) {
  ...
  const idsToStacktraces = Object.fromEntries(
    trackedStacktracesArray.map(({ id, stacktraces }) => [id, stacktraces])
  );
  ...
  return await promisePool(
    ...
    collections.map((collection) => async () => {
      ...
      if (collection.id in idsToStacktraces) {
        const stacktraces = idsToStacktraces[collection.id].slice(0, 50);
        if (idsToStacktraces[collection.id].length > 50) {
          console.log("for test, only analysis first 50 items.");
        }
        res.stacktraces = await Promise.all(
          stacktraces.map(async (stacktrace) => {
